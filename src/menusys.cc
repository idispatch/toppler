...
_menusystem *new_menu_system(const char *title, menuopt_callback_proc pr, int molen, int ystart) {
    ...
    if (ms) {
        ...
        if (title) {
            memcpy(ms->title, title,
                    (strlen(title) < MENUTITLELEN) ? strlen(title) + 1 : (MENUTITLELEN - 1));
        }
...
_menusystem *
add_menu_option(_menusystem *ms, const char *name, menuopt_callback_proc pr, SDLKey quickkey,
        menuoptflags flags, int state) {
    _menuoption *tmp;
    int olen = 0;
    ...
    if (name) {
        olen = strlen(name);
        memcpy(tmp[ms->numoptions].oname, name,
...
void draw_menu_system(_menusystem *ms, Uint16 dx, Uint16 dy) {
    static int color_r = 0, color_g = 20, color_b = 70;

    if (!ms)
        return;

    int y, offs = 0, len, realy, minx, miny, maxx, maxy, scrlen, newhilite = -1, yz, titlehei;
...
void menu_system_caller(_menusystem *ms) {
...
run_menu_system(_menusystem *ms, _menusystem *parent) {
    ...
    do {
        ...
        if ((ms->opt_steal_control >= 0) && (ms->opt_steal_control < ms->numoptions)
...
void men_info(const char *s, long timeout, int fire) {
    ...
    do {
        ...
        if (!timeout)
            ende = true;
        else if ((fire == 2) && (key_chartyped() == ' '))
            ende = true;
        else if ((fire != 2) && key_keypressed(fire_key))
            ende = true;
    } while (!ende);
    (void) key_sdlkey();
}
...
bool men_input(char *origs, int max_len, int xpos, int ypos, const char *allowed) {
    ...
    case SDLK_DELETE:
        if (strlen(s) >= (unsigned) pos) {
            for (ztmp = pos; ztmp < max_len - 1; ztmp++)
                s[ztmp] = s[ztmp + 1];
            s[ztmp] = '\0';
        }
        break;
    case SDLK_BACKSPACE:
        if (pos > 0) {
            if (pos <= max_len) {
                for (ztmp = pos - 1; ztmp < max_len - 1; ztmp++)
                    s[ztmp] = s[ztmp + 1];
                s[ztmp] = '\0';
            }
            pos--;
    ...
    default:
        ...
        if (allowed) {
            if (!strchr(allowed, inpc)) {
                if (strchr(allowed, toupper(inpc)))
                    inpc = toupper(inpc);
                else if (strchr(allowed, tolower(inpc)))
                    inpc = tolower(inpc);
                else
                    break;
            }
        ...
        if ((strlen(s) >= (unsigned) pos) && (strlen(s) < (unsigned) max_len)) {
            for (ztmp = max_len - 1; ztmp >= pos; ztmp--)
                s[ztmp + 1] = s[ztmp];
            s[pos] = inpc;
            s[max_len] = '\0';
            pos++;
