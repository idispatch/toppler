...
static void men_reload_sprites(Uint8 what) {
    ...
    if (what & 2) {
        ...
        scr_read_palette(&fi, pal);
        titledata = scr_loadsprites(&fontsprites, &fi, 1, SPR_TITLEWID, SPR_TITLEHEI, true, pal,
                config.use_alpha_font());
    }
}
...
static const char *game_options_menu_statustop(_menusystem *prevmenu) {
    ...
    if (config.status_top())
        sprintf(txt, "%s %c", _("Status on top"), 4);
    else
        sprintf(txt, "%s %c", _("Status on top"), 3);
...
static const char *game_options_menu_lives(_menusystem *prevmenu) {
    ...
    if (prevmenu) {
        ...
        case right_key:
            ...
            if (config.start_lives() > 3)
                config.start_lives(3);
            break;
        case left_key:
            config.start_lives(config.start_lives() - 1);
            if (config.start_lives() < 1)
                config.start_lives(1);
            break;
        default:
            return NULL;
        }
...
game_options_menu_speed(_menusystem *prevmenu) {
    ...
    if (prevmenu) {
        ...
        case right_key:
            ...
            if (config.game_speed() > MAX_GAME_SPEED)
                config.game_speed(MAX_GAME_SPEED);
            break;
        case left_key:
            config.game_speed(config.game_speed() - 1);
...
game_options_bonus(_menusystem *ms) {
    ...
    if (config.nobonus())
        sprintf(txt, "%s %c", _("Bonus"), 3);
    else
        sprintf(txt, "%s %c", _("Bonus"), 4);
...
men_options_windowed(_menusystem *ms) {
    ...
    if (config.fullscreen())
        sprintf(txt, "%s %c", _("Fullscreen"), 4);
    else
        sprintf(txt, "%s %c", _("Fullscreen"), 3);
...
men_alpha_font(_menusystem *ms) {
    ...
    if (config.use_alpha_font())
        sprintf(txt, "%s %c", _("Font alpha"), 4);
    else
        sprintf(txt, "%s %c", _("Font alpha"), 3);
...
men_alpha_sprites(_menusystem *ms) {
    ...
    if (config.use_alpha_sprites())
        sprintf(txt, "%s %c", _("Sprites alpha"), 4);
    else
        sprintf(txt, "%s %c", _("Sprites alpha"), 3);
...
men_alpha_layer(_menusystem *ms) {
    ...
    if (config.use_alpha_layers())
        sprintf(txt, "%s %c", _("Scroller alpha"), 4);
    else
        sprintf(txt, "%s %c", _("Scroller alpha"), 3);
...
men_alpha_menu(_menusystem *ms) {
    ...
    if (config.use_alpha_darkening())
        sprintf(txt, "%s %c", _("Shadowing"), 4);
    else
        sprintf(txt, "%s %c", _("Shadowing"), 3);
...
men_waves_menu(_menusystem *ms) {
    if (ms) {
        ...
        case right_key:
            ...
            if (config.waves_type() >= configuration::num_waves)
                config.waves_type(configuration::num_waves - 1);
            break;
        case left_key:
            config.waves_type(config.waves_type() - 1);
            if (config.waves_type() < 0)
                config.waves_type(0);
            break;
        default:
            return NULL;
    ...
    case configuration::waves_nonreflecting:
        return _("Nonreflecting waves");
    case configuration::waves_simple:
        return _("Simple waves");
    case configuration::waves_expensive:
        return _("Expensive waves");
    default:
        return _("Error");
    }
}
...
static void get_hiscores_string(int p, char **pos, char **points, char **name) {
    Uint32 pt;
    Uint8 tw;
...
static void calc_hiscores_maxlen(int *max_pos, int * max_points, int *max_name) {
...
men_hiscores_background_proc(_menusystem *ms) {
    ...
    if (ms) {
        ...
        for (int t = 0; t < HISCORES_PER_PAGE; t++) {
            ...
            scr_writetext(
                    hiscores_xpos + hiscores_maxlen_pos + 20 + hiscores_maxlen_points
                            - scr_textlength(points), ypos, points);
            scr_writetext(hiscores_xpos + hiscores_maxlen_pos + 20 + 20 + hiscores_maxlen_points,
                    ypos, name);
        }
...
static void show_scores(bool back = true, int mark = -1) {
    ...
    for (int tmpz = 0; tmpz < HISCORES_PER_PAGE; tmpz++)
        ms = add_menu_option(ms, NULL, NULL);
...
men_main_startgame_proc(_menusystem *ms) {
    if (ms) {
        ...
        case fire_key:
            ...
            break;
        case right_key:
            currentmission = (currentmission + 1) % missioncount;
            break;
        case left_key:
            currentmission = (currentmission + missioncount - 1) % missioncount;
            break;
        default:
            return NULL;
        }
