...
static int testunderground(int verticalpos, int anglepos, bool look_left) {
    static unsigned char unter[32] = { 0x11, 0x20, 0x02, 0x00, 0x11, 0x00, 0x32, 0x00, 0x11, 0x00,
            0x32, 0x00, 0x11, 0x00, 0x11, 0x00, 0x11, 0x00, 0x11, 0x00, 0x11, 0x00, 0x11, 0x00,
            0x11, 0x23, 0x00, 0x00, 0x11, 0x23, 0x00, 0x00 };
    ...
    if ((!lev_is_empty(r, c)) && (!lev_is_door(r, c)))
        erg++;
...
void top_updatetoppler(int left_right, int up_down, bool space) {
    ...
    /* table specifying the number for the animal sprite if it is turning
     around */
    static unsigned char umdreh[7] = { 0x8, 0x9, 0xa, 0xb, 0xa, 0x9, 0x8 };

    /* the sprites if the animal turns to enter the door */
    static unsigned char door1[4] = { 0x10, 0x11, 0x12, 0x13 };

    /* and when it enters the door */
    static unsigned char door2[6] = { 0x13, 0x14, 0x14, 0x15, 0x15, 0x16 };

    /* when it leaves the door */
    static unsigned char door3[6] = { 0x17, 0x18, 0x18, 0x19, 0x19, 0xb };
    ...
    static unsigned char schiessen[3] = { 0x00, 0x1a, 0x1b };

    /* the sprites for toppling over */
    static unsigned char toppler1[4] = { 0x00, 0x1c, 0x1d, 0x1e };
        ...
        case 0:
            if (left_right == 0) {
                if (space)
                    ...
                    if (lev_is_up_station(verticalpos / 4 - 1, anglepos / 8) && (up_down == 1)) {
                        elevator(up_down);
                        break;
                    }
                    if (lev_is_down_station(verticalpos / 4 - 1, anglepos / 8) && (up_down == -1)) {
                        elevator(up_down);
                        break;
                    }

                    if (lev_is_door_upperend(verticalpos / 4, anglepos / 8) && (up_down == 1)) {
    ...
    case STATE_DROWN:
        if (substate == 0x8)
            ttsounds::instance()->startsound(SND_DROWN);
        if (substate < 0x18) {
...
void top_testcollision(void) {
    ...
    if ((state == STATE_TOPPLING) || (state == STATE_DROWN)
            || ((state == STATE_DOOR) && (substate >= 10) && (substate < 31)) || (!tvisible))
        return;
...
bool top_died(void) {
    return state == STATE_DROWNED;
}
bool top_targetreached(void) {
    return state == STATE_FINISHED;
}
bool top_ended(void) {
    return ((state == STATE_DROWNED) || (state == STATE_FINISHED));
}
bool top_dying(void) {
    return ((state == STATE_DROWN) || (state == STATE_DROWNED) || (state == STATE_FINISHED));
}
...
void top_sidemove(void) {
    ...
    if ((state != STATE_STANDING) && (state != STATE_JUMPING) && (state != STATE_TURNING)
            && (state != STATE_SHOOTING))
        return;
