...
typedef enum {
    STATE_PLAYING, STATE_ABORTED, STATE_DIED, STATE_TIMEOUT, STATE_FINISHED
} gam_states;
...
/* updates the position of the tower on screen
 ...
 of the tower */
static unsigned short towerpos(int verticalpos, int &tower_position, int anglepos,
        int &tower_angle) {
    int i, j;
...
static void game_background_proc(void) {
    scr_drawall(towerpos(top_verticalpos(), bg_tower_pos, top_anglepos(), bg_tower_angle),
            (4 - top_anglepos()) & 0x7f, bg_time, false, 0, 0, SF_NONE);
...
static void writebonus(int &tower_position, int tower_anglepos, int zeit, int tec, int extra,
        int time, int lif, bool lifes) {
    char s[30];

    scr_drawall(towerpos(top_verticalpos(), tower_position, top_anglepos(), tower_anglepos),
            (4 - top_anglepos()) & 0x7f, time, false, 0, 0, SF_NONE);
...
static void escape(gam_states &state, int &tower_position, int &tower_anglepos, int time) {
    ...
    towerpos(top_verticalpos(), tower_position, top_anglepos(), tower_anglepos);
}
...
gam_result gam_towergame(Uint8 &anglepos, Uint16 &resttime, int &demo, void *demobuf) {
    ...
    assert_msg(!(((demo == -1) || (demo > 0)) && !demobuf),
            "Trying to play or record a null demo.");
    ...
    do {
        ...
        scr_drawall(towerpos(top_verticalpos(), tower_position, top_anglepos(), tower_angle),
                (4 - top_anglepos()) & 0x7f, time, false, 0, 0, drawflags);
        scr_swap();
        ttsounds::instance()->play();
        dcl_wait();
